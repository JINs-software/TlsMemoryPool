...
private:	// private 생성자 -> 임의의 생성을 막는다. 
	...
	TlsMemPool(size_t unitCnt, size_t capacity, bool placementNew = false, bool referenceFlag = false);
	~TlsMemPool();
...
template<typename T>
TlsMemPool<T>::TlsMemPool(size_t unitCnt, size_t capacity, bool placementNew, bool referenceFlag)
	: m_FreeFront(NULL), m_UnitCnt(unitCnt), m_Capacity(capacity), m_PlacementNewFlag(placementNew), m_ReferenceFlag(referenceFlag)
{
	if (m_Capacity > 0) {
...
public:
	...
	DWORD AllocTlsMemPool(size_t memPoolUnitCnt, size_t memPoolCapacity, bool placementNewFlag, bool referenceFlag);
...
template<typename T>
inline DWORD TlsMemPoolManager<T>::AllocTlsMemPool(size_t memPoolUnitCnt, size_t memPoolCapacity, bool placementNewFlag, bool referenceFlag)
{
	if (TlsGetValue(m_TlsIMainIndex) == NULL) {
		// TlsMemPool 생성
		TlsMemPool<T>* newTlsMemPool = new TlsMemPool<T>(memPoolUnitCnt, memPoolCapacity, placementNewFlag, referenceFlag);
		if (newTlsMemPool == NULL) {
			DebugBreak();
		}
